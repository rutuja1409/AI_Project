...
def define_dataloader(logger, opt):
    ...
    if opt["global_rank"] == 0 and val_dataset is not None:
        val_dataloader_args = opt["datasets"]["validation"]["dataloader"][
            "args"
        ]  # Use val_args from validation section
        val_dataloader = DataLoader(
            val_dataset, worker_init_fn=worker_init_fn, **val_dataloader_args
        )
    else:
        val_dataloader = None
...
def define_dataset(logger, opt):
    ...
    phase_dataset = init_obj(
        dataset_opt, logger, default_file_name="data.dataset", init_type="Dataset"
    )
    ...
    logger.info(
        "Dataset for {} have {} samples.".format(opt["phase"], len(phase_dataset))
    )

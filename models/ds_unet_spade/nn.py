...
class SPADE(nn.Module):
    ...
    def forward(self, noise, cond_image):
        # Part 1. generate parameter-free normalized activations
        normalized = self.param_free_norm(noise)

        # Part 2. produce scaling and bias conditioned on semantic map
        cond_image = F.interpolate(cond_image, size=noise.size()[2:], mode="nearest")
        actv = self.mlp_shared(cond_image)

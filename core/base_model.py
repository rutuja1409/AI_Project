...
class BaseModel:
    def __init__(self, opt, phase_loader, val_loader, metrics, logger, writer):
        ...
        """ process record """
        self.batch_size = self.opt["datasets"][self.phase]["dataloader"]["args"][
            "batch_size"
        ]
        self.epoch = 0
        self.iter = 0
    ...
    def train(self):
        while (
            ...
            and self.iter <= self.opt["train"]["n_iter"]
        ):
            self.epoch += 1
            ...
            if self.epoch % self.opt["train"]["save_checkpoint_epoch"] == 0:
                self.logger.info(
                    "Saving the self at the end of epoch {:.0f}".format(self.epoch)
                )
            ...
            if self.epoch % self.opt["train"]["val_epoch"] == 0:
                ...
                if self.val_loader is None:
                    self.logger.warning(
                        "Validation stop where dataloader is None, Skip it."
                    )
                else:
    ...
    def load_network(self, network, network_label, strict=True):
        if self.opt["path"]["resume_state"] is None:
            return
        self.logger.info(
            "Beign loading pretrained model [{:s}] ...".format(network_label)
        )
        ...
        if not os.path.exists(model_path):
            self.logger.warning(
                "Pretrained model in [{:s}] is not existed, Skip it".format(model_path)
            )
            return
        ...
        network.load_state_dict(
            torch.load(
                model_path, map_location=lambda storage, loc: Util.set_device(storage)
            ),
            strict=strict,
        )
    ...
    def resume_training(self):
        ...
        if not os.path.exists(state_path):
            self.logger.warning(
                "Training state in [{:s}] is not existed, Skip it".format(state_path)
            )
            return

        self.logger.info("Loading training state for [{:s}] ...".format(state_path))
        resume_state = torch.load(
            state_path, map_location=lambda storage, loc: self.set_device(storage)
        )
        ...
        assert len(resume_optimizers) == len(
            self.optimizers
        ), "Wrong lengths of optimizers {} != {}".format(
            len(resume_optimizers), len(self.optimizers)
        )
        assert len(resume_schedulers) == len(
            self.schedulers
        ), "Wrong lengths of schedulers {} != {}".format(
            len(resume_schedulers), len(self.schedulers)
        )

    ...
    **modify_kwargs
    ...
    try:
        ...
        if isinstance(attr, type):
            ...
            # ret = attr
        logger.info(
            "{} [{:s}() form {:s}] is created.".format(init_type, class_name, file_name)
        )
    except:
        raise NotImplementedError(
            "{} [{:s}() form {:s}] not recognized.".format(
                init_type, class_name, file_name
            )
        )
    return ret
...
def parse(args):
    ...
    experiments_root = os.path.join(
        opt["path"]["base_dir"], "{}_{}".format(opt["name"], get_timestamp())
    )
